...
const StepHike = ({ children, title }) => {
  ...
  const steps = children.filter((x) => x.type.name === "Step");
  ...
  return (
    <div className="">
     ...
     -mb-1.5
      ...
      <StepHikeContext.Provider value={{ activeStep, steps }}>
        {children}
      </StepHikeContext.Provider>
    </div>
  );
};
  ...
  return (
    <StepHikeContext.Consumer>
      {({ activeStep, steps }) => {
        ...
        return (
          <div>
            ...
            <div
              ref={ref}
              className="bg-white-1200 dark:bg-whiteA-300 border-scale-600 not-prose sticky top-[64px]

      z-10 flex h-[60px]
      w-full items-center gap-3 rounded-lg


      border-b border-l border-r border-t p-5
      ...
      "
            >
              <div className="flex items-center gap-6">
                <div className="bg-scale-600 border-scale-700 text-scale-1200 flex h-7 w-7 items-center justify-center rounded border font-mono text-base font-semibold">
                  {step}
                </div>
